...
function ChatWithExpert() {
  ...
  async function fetchExpert() {
    const res = await axios.get("/API/pharmacist/find-expert", {
      params: { mobile },
      withCredentials: true,
    });

    if (res.data.success) setName(res.data.expert.name);
  }
  ...
  useEffect(() => {
    ...
    mySocket.on('login', (mobile) => {
      console.log("Expert Online: " + mobile);
  ...
  const sendMessage = () => {
    ...
    mySocket.emit("message", { message, from, to });
    setMessage("");
  };
